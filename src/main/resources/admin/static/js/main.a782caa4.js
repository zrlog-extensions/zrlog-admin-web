(()=>{"use strict";var e={3127:(e,t,o)=>{o.r(t),o.d(t,{default:()=>a});var r=o(67718),n=o(44414);const a=e=>{let{code:t,data:o,style:a}=e;return(0,n.jsx)(r.Ay,{status:t,title:t,subTitle:o.message,style:a})}},11113:(e,t,o)=>{o.d(t,{$L:()=>d,Ay:()=>g,TX:()=>l,l6:()=>c,mX:()=>a});var r=o(85221),n=o(44414);const a="ss_key",i="__SS_DATA__",s="__SS_PAGE_BUILD_ID__",l=()=>window[i],d=()=>window[s],c=e=>{window[s]=e},g=e=>{let{children:t}=e;const o=()=>{var e;const t=null===(e=document.getElementById(i))||void 0===e?void 0:e.innerText;let o;if(o=(null===t||void 0===t?void 0:t.length)>0?JSON.parse(t):{key:"",data:void 0,resourceInfo:{},user:null,pageBuildId:"",systemNotification:""},""===o.key||null===o.key||void 0===o.key){const e=localStorage.getItem(a);e&&(o.key=e)}return o};window[i]=o(),window[s]=o().pageBuildId;const d=l();return void 0!==d.user&&null!==d.user||""!==d.key&&(d.user=(0,r.Pn)("/user")),(0,n.jsx)(n.Fragment,{children:t})}},15246:(e,t,o)=>{const r=Boolean("localhost"===window.location.hostname||"[::1]"===window.location.hostname||window.location.hostname.match(/^127(?:\.(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)){3}$/));function n(e,t){navigator.serviceWorker.register(e).then(e=>{e.onupdatefound=()=>{const o=e.installing;null!=o&&(o.onstatechange=()=>{"installed"===o.state&&(navigator.serviceWorker.controller?(console.log("New content is available and will be used when all tabs for this page are closed. See https://bit.ly/CRA-PWA."),t&&t.onUpdate&&t.onUpdate(e)):(console.log("Content is cached for offline use."),t&&t.onSuccess&&t.onSuccess(e)))})}}).catch(e=>{console.error("Error during service worker registration:",e)})}var a=o(1352),i=o(11113),s=o(77163),l=o(44414);const d=()=>(0,l.jsx)(i.Ay,{children:(0,l.jsx)(s.Ay,{})}),c=document.getElementById("app");(0,a.H)(c).render((0,l.jsx)(d,{})),function(e){if("serviceWorker"in navigator){if(new URL("/admin",window.location.href).origin!==window.location.origin)return;window.addEventListener("load",()=>{const t="admin/service-worker.js?v=20250724";r?(!function(e,t){fetch(e,{headers:{"Service-Worker":"script"}}).then(o=>{const r=o.headers.get("content-type");404===o.status||null!=r&&-1===r.indexOf("javascript")?navigator.serviceWorker.ready.then(e=>{e.unregister().then(()=>{window.location.reload()})}):n(e,t)}).catch(()=>{console.log("No internet connection found. AppBase is running in offline mode.")})}(t,e),navigator.serviceWorker.ready.then(()=>{console.log("This web app is being served cache-first by a service worker. To learn more, visit https://bit.ly/CRA-PWA")})):n(t,e)})}}()},23236:(e,t,o)=>{o.d(t,{A:()=>s});var r=o(23072),n=o(44361),a=o(77163),i=o(44414);const s=e=>{let{title:t}=e;return(0,i.jsxs)(i.Fragment,{children:[(0,i.jsx)(n.A,{className:"page-header",level:3,style:{borderLeft:`3px solid ${(0,a.BV)().colorPrimary}`},children:t}),(0,i.jsx)(r.A,{})]})}},26044:(e,t,o)=>{o.d(t,{A:()=>c});var r=o(24937);const n=(0,r.Ay)("div")`
pre code.hljs {
    display: block;
    overflow-x: auto;
    padding: 1em
}
code.hljs {
    padding: 3px 5px
}
/*!
  Theme: GitHub Dark
  Description: Dark theme as seen on github.com
  Author: github.com
  Maintainer: @Hirse
  Updated: 2021-05-15

  Outdated base version: https://github.com/primer/github-syntax-dark
  Current colors taken from GitHub's CSS
*/
.hljs {
    color: #c9d1d9;
    background: #0d1117
}
.hljs-doctag,
.hljs-keyword,
.hljs-meta .hljs-keyword,
.hljs-template-tag,
.hljs-template-variable,
.hljs-type,
.hljs-variable.language_ {
    /* prettylights-syntax-keyword */
    color: #ff7b72
}
.hljs-title,
.hljs-title.class_,
.hljs-title.class_.inherited__,
.hljs-title.function_ {
    /* prettylights-syntax-entity */
    color: #d2a8ff
}
.hljs-attr,
.hljs-attribute,
.hljs-literal,
.hljs-meta,
.hljs-number,
.hljs-operator,
.hljs-variable,
.hljs-selector-attr,
.hljs-selector-class,
.hljs-selector-id {
    /* prettylights-syntax-constant */
    color: #79c0ff
}
.hljs-regexp,
.hljs-string,
.hljs-meta .hljs-string {
    /* prettylights-syntax-string */
    color: #a5d6ff
}
.hljs-built_in,
.hljs-symbol {
    /* prettylights-syntax-variable */
    color: #ffa657
}
.hljs-comment,
.hljs-code,
.hljs-formula {
    /* prettylights-syntax-comment */
    color: #8b949e
}
.hljs-name,
.hljs-quote,
.hljs-selector-tag,
.hljs-selector-pseudo {
    /* prettylights-syntax-entity-tag */
    color: #7ee787
}
.hljs-subst {
    /* prettylights-syntax-storage-modifier-import */
    color: #c9d1d9
}
.hljs-section {
    /* prettylights-syntax-markup-heading */
    color: #1f6feb;
    font-weight: bold
}
.hljs-bullet {
    /* prettylights-syntax-markup-list */
    color: #f2cc60
}
.hljs-emphasis {
    /* prettylights-syntax-markup-italic */
    color: #c9d1d9;
    font-style: italic
}
.hljs-strong {
    /* prettylights-syntax-markup-bold */
    color: #c9d1d9;
    font-weight: bold
}
.hljs-addition {
    /* prettylights-syntax-markup-inserted */
    color: #aff5b4;
    background-color: #033a16
}
.hljs-deletion {
    /* prettylights-syntax-markup-deleted */
    color: #ffdcd7;
    background-color: #67060c
}
.hljs-char.escape_,
.hljs-link,
.hljs-params,
.hljs-property,
.hljs-punctuation,
.hljs-tag {
    /* purposely ignored */

}
`,a=(0,r.Ay)("div")`
/*!
  Theme: Default
  Description: Original highlight.js style
  Author: (c) Ivan Sagalaev <maniac@softwaremaniacs.org>
  Maintainer: @highlightjs/core-team
  Website: https://highlightjs.org/
  License: see project LICENSE
  Touched: 2021
*/
/*
This is left on purpose making default.css the single file that can be lifted
as-is from the repository directly without the need for a build step

Typically this "required" baseline CSS is added by \`makestuff.js\` during build.
*/

pre code.hljs {
    display: block;
    overflow-x: auto;
    padding: 1em
}

code.hljs {
    padding: 3px 5px
}

/* end baseline CSS */

.hljs {
    background: #F3F3F3!important;
    color: #444
}

/* Base color: saturation 0; */

.hljs-subst {
    /* default */

}

/* purposely ignored */

.hljs-formula,
.hljs-attr,
.hljs-property,
.hljs-params {

}

.hljs-comment {
    color: #697070
}

.hljs-tag,
.hljs-punctuation {
    color: #444a
}

.hljs-tag .hljs-name,
.hljs-tag .hljs-attr {
    color: #444
}

.hljs-keyword,
.hljs-attribute,
.hljs-selector-tag,
.hljs-meta .hljs-keyword,
.hljs-doctag,
.hljs-name {
    font-weight: bold
}

/* User color: hue: 0 */

.hljs-type,
.hljs-string,
.hljs-number,
.hljs-selector-id,
.hljs-selector-class,
.hljs-quote,
.hljs-template-tag,
.hljs-deletion {
    color: #880000
}

.hljs-title,
.hljs-section {
    color: #880000;
    font-weight: bold
}

.hljs-regexp,
.hljs-symbol,
.hljs-variable,
.hljs-template-variable,
.hljs-link,
.hljs-selector-attr,
.hljs-operator,
.hljs-selector-pseudo {
    color: #ab5656
}

/* Language color: hue: 90; */

.hljs-literal {
    color: #695
}

.hljs-built_in,
.hljs-bullet,
.hljs-code,
.hljs-addition {
    color: #397300
}

/* Meta color: hue: 200 */

.hljs-meta {
    color: #1f7199
}

.hljs-meta .hljs-string {
    color: #38a
}

/* Misc effects */

.hljs-emphasis {
    font-style: italic
}

.hljs-strong {
    font-weight: bold
}
`;var i=o(9950),s=o(33730),l=o(77163),d=o(44414);const c=e=>{let{htmlContent:t,previewRef:o,editable:r,onContentChange:c,style:g}=e;const h=(0,i.useRef)(null);(0,i.useEffect)(()=>{h.current&&h.current.innerHTML!==t&&(h.current.innerHTML=t)},[t]);const A={ref:h,autoFocus:!1,contentEditable:r,className:"markdown-body",onInput:()=>{c&&h.current&&c(h.current.innerHTML)},style:{outline:"none",boxShadow:"none"}};return(0,d.jsx)(s.Q,{ref:o,style:g,children:(0,l.BV)().dark?(0,d.jsx)(n,{...A}):(0,d.jsx)(a,{...A})})}},31743:(e,t,o)=>{o.d(t,{A:()=>d});var r=o(9950),n=o(28429),a=o(85221),i=o(37596),s=o(35434),l=o(44414);const d=e=>{let{children:t}=e;const o=(0,n.zy)(),d=(0,n.Zp)(),c="true"===sessionStorage.getItem("loaded")||!(0,s.x)(),[g,h]=(0,r.useState)(c);return(0,r.useEffect)(()=>{if(g)return;const e=(0,a.HB)();return sessionStorage.setItem("loaded","true"),e&&e!==(0,i.dy)(o)?(console.log((0,i.dy)(o)+" redirecting to last opened page:",e),void d(e)):void 0},[]),(0,r.useEffect)(()=>{if(!(0,s.x)())return;const e=(0,i.dy)(o);if(!g){const t=(0,a.HB)();return void((void 0===t||null===t||t===e)&&h(!0))}"true"===sessionStorage.getItem("loaded")&&(e.startsWith("/500")||e.startsWith("/404")||e.startsWith("/403")||e.startsWith("/offline")||(0,a.cw)(e))},[o]),g?(0,l.jsx)(l.Fragment,{children:t}):(0,l.jsx)(l.Fragment,{})}},33730:(e,t,o)=>{o.d(t,{Q:()=>r});const r=(0,o(24937).Ay)("div")`
    .markdown-body strong {
        font-weight: 700;
    }

    .markdown-body h1 {
        margin: 0.67rem 0;
    }

    .markdown-body img {
        border: 0;
    }

    .markdown-body hr {
        box-sizing: content-box;
        height: 0;
    }

    .markdown-body input {
        color: inherit;
        margin: 0;
        line-height: normal;
    }

    .markdown-body html input[disabled] {
        cursor: default;
    }

    .markdown-body input[type="checkbox"] {
        box-sizing: border-box;
        padding: 0;
    }

    .markdown-body * {
        box-sizing: border-box;
    }

    .markdown-body a {
        background: 0 0;
        text-decoration: none;
    }

    .markdown-body a:active,
    .markdown-body a:hover {
        outline: 0;
        text-decoration: underline;
    }

    .markdown-body hr {
        margin: 12px 0;
        overflow: hidden;
        background: 0 0;
        border: 0;
        border-bottom: 1px solid #ddd;
    }

    .markdown-body h1,
    .markdown-body h2 {
        padding-bottom: 0.3rem;
        border-bottom: 1px solid #eee;
    }

    .markdown-body blockquote {
        margin: 0;
    }

    .markdown-body ol ol,
    .markdown-body ul ol {
        list-style-type: lower-roman;
    }

    .markdown-body ol ol ol,
    .markdown-body ol ul ol,
    .markdown-body ul ol ol,
    .markdown-body ul ul ol {
        list-style-type: lower-alpha;
    }

    .markdown-body dd {
        margin-left: 0;
    }

    .markdown-body > :first-child {
        margin-top: 0 !important;
    }

    .markdown-body > :last-child {
        margin-bottom: 0 !important;
    }

    .markdown-body h1,
    .markdown-body h2,
    .markdown-body h3,
    .markdown-body h4,
    .markdown-body h5,
    .markdown-body h6 {
        position: relative;
        margin-top: 1rem;
        margin-bottom: 1rem;
        font-weight: 700;
        line-height: 1.4;
    }

    .markdown-body h1 {
        font-size: 2.25rem;
        line-height: 1.2;
    }

    .markdown-body h2 {
        font-size: 1.75rem;
        line-height: 1.225;
    }

    .markdown-body h3 {
        font-size: 1.5rem;
        line-height: 1.43;
    }

    .markdown-body h4 {
        font-size: 1.25rem;
    }

    .markdown-body h5 {
        font-size: 1rem;
    }

    .markdown-body h6 {
        font-size: 1rem;
        color: #777;
    }

    .markdown-body blockquote,
    .markdown-body dl,
    .markdown-body ol,
    .markdown-body p,
    .markdown-body pre,
    .markdown-body table,
    .markdown-body ul {
        margin-top: 0;
        margin-bottom: 1rem;
    }

    .markdown-body ol,
    .markdown-body ul {
        padding-left: 2rem;
    }

    .markdown-body ol ol,
    .markdown-body ol ul,
    .markdown-body ul ol,
    .markdown-body ul ul {
        margin-top: 0;
        margin-bottom: 0;
    }

    .markdown-body li > p {
        margin-top: 1rem;
    }

    .markdown-body dl {
        padding: 0;
    }

    .markdown-body dl dt {
        padding: 0;
        margin-top: 1rem;
        font-size: 1rem;
        font-style: italic;
        font-weight: 700;
    }

    .markdown-body dl dd {
        padding: 0 1rem;
        margin-bottom: 1rem;
    }

    .markdown-body blockquote {
        padding: 0 12px;
        color: #777;
        border-left: 4px solid #ddd;
    }

    .markdown-body blockquote > :first-child {
        margin-top: 0;
    }

    .markdown-body blockquote > :last-child {
        margin-bottom: 0;
    }

    .markdown-body table {
        border-collapse: collapse;
        border-spacing: 0;
        display: block;
        width: 100%;
        overflow: auto;
        word-break: normal;
    }

    .markdown-body table th {
        font-weight: 700;
    }

    .markdown-body table td,
    .markdown-body table th {
        padding: 6px 13px;
        border: 1px solid #ddd;
    }

    .markdown-body table tr {
        background-color: #fff;
        border-top: 1px solid #ccc;
    }

    .markdown-body table tr:nth-child(2n) {
        background-color: #f8f8f8;
    }

    .markdown-body img {
        max-width: 100%;
        box-sizing: border-box;
    }

    .markdown-body code {
        padding: 0.2rem 0;
        margin: 0;
        background-color: rgba(0, 0, 0, 0.04);
        border-radius: 8px;
    }

    .markdown-body pre > code {
        margin: 4px;
        font-size: 12px;
        word-break: normal;
        white-space: pre;
        background: 0 0;
        border: 0;
    }

    .markdown-body kbd {
        display: inline-block;
        padding: 3px 5px;
        line-height: 10px;
        color: #555;
        vertical-align: middle;
        background-color: #fcfcfc;
        border: 1px solid #ccc;
        border-bottom-color: #bbb;
        border-radius: 8px;
        box-shadow: inset 0 -1px 0 #bbb;
    }

    .markdown-body .task-list-item input {
        float: left;
        margin: 0.3rem 0 0.25rem -1.6rem;
        vertical-align: middle;
    }

    .markdown-body .highlight pre,
    .markdown-body pre {
        line-height: 1.6;
        font-size: 85%;
        border-radius: 8px;
    }
`},34647:(e,t,o)=>{o.d(t,{Ay:()=>j,_Z:()=>b,cz:()=>f});var r=o(28429),n=o(9950),a=o(20212),i=o(71420),s=o(29246),l=o(38692),d=o(3127),c=o(44414);const g=e=>{let{children:t}=e;const[o,r]=(0,n.useState)(!1);return(0,n.useEffect)(()=>{const e=()=>{r(!0)};return window.addEventListener("error",e),()=>{window.removeEventListener("error",e)}},[]),o?(0,c.jsx)(d.default,{data:{message:"Something went wrong. Please refresh the page."},code:"500"}):(0,c.jsx)(c.Fragment,{children:t})};var h=o(78816),A=o(63251),u=o(11113);const m=(0,n.lazy)(()=>Promise.all([o.e(4121),o.e(2816),o.e(4868)]).then(o.bind(o,4868))),w=e=>{let{offline:t}=e;const o=f(),a=(0,r.Zp)(),s=(0,u.TX)().user,[l,d]=(0,n.useState)(s);return(0,n.useEffect)(()=>{t||void 0!==l&&null!==l||(0,A.b)(`/user/info?_t=${(new Date).getTime()}`,o).then(e=>{if(e&&0===e.error){e.key&&((0,u.TX)().key=e.key);const t=e.data;(0,u.TX)().user=t,d(t)}else b(a)}).catch(()=>{b(a)})},[]),void 0===l||null===l?(0,c.jsx)(i.A,{fullscreen:!0,delay:1e3}):(0,c.jsx)(m,{offline:t,userInfo:l})},p=(0,n.lazy)(()=>Promise.resolve().then(o.bind(o,59922))),y=new Map,b=e=>{window.location.search.includes("redirectFrom")||e((0,h.dW)(`/login?redirectFrom=${encodeURIComponent(window.location.pathname.split(".html")[0])}${encodeURIComponent(window.location.search)}`),{replace:!0})},f=e=>{const{modal:t,message:o}=a.A.useApp(),n=(0,r.Zp)(),i=s.A.create();(0,h.M$)()&&(i.defaults.withCredentials=!0);return i.defaults.baseURL=(0,h.XW)(),i.interceptors.response.use(o=>{const r=o.data.error;if(9001===r){let a=y.get(r);return null!==a&&void 0!==a||(a=0),0===a&&(y.set(r,a+1),t.error({title:o.data.error,content:o.data.message,getContainer:e?e():void 0,onOk:()=>{y.set(r,0)}})),(0,h.M$)()&&b(n),Promise.reject(o.data)}return o},r=>(r=>{if(r&&r.config&&r.config.url){if(r.config.url.includes(l.API_VERSION_PATH))return Promise.reject(r.message);if(r.config.url.includes(l.API_DO_UPGRADE_PATH))return Promise.reject(r.message)}if(r&&r.response){if(r.response.status)return t.error({title:"\u670d\u52a1\u5f02\u5e38["+r.response.status+"]",content:(0,c.jsx)("div",{style:{paddingTop:20,overflow:"auto"},dangerouslySetInnerHTML:{__html:r.response.data}}),getContainer:e?e():void 0}),Promise.reject(r.response)}else r&&r.config&&r.config.url&&(navigator.onLine?t.error({title:"\u8bf7\u6c42 "+r.config.url+" \u9519\u8bef",content:r.message,getContainer:e?e():void 0}):o.error("\u8bf7\u6c42 "+r.config.url+" "+r.toString()+" network offline"));return Promise.reject(r)})(r)),i},j=e=>{let{offline:t}=e;return(0,c.jsxs)(r.BV,{children:[["login","login.html"].map(e=>(0,c.jsx)(r.qh,{path:e,element:(0,c.jsx)(g,{children:(0,c.jsx)(n.Suspense,{fallback:(0,c.jsx)(i.A,{spinning:!0,fullscreen:!0,delay:1e3}),children:(0,c.jsx)(p,{offline:t})})})},e)),(0,c.jsx)(r.qh,{path:"logout",element:(0,c.jsx)(g,{children:(0,c.jsx)(n.Suspense,{fallback:(0,c.jsx)(i.A,{spinning:!0,fullscreen:!0,delay:1e3}),children:(0,c.jsx)(p,{offline:t})})})}),(0,c.jsx)(r.qh,{path:"*",element:(0,c.jsx)(g,{children:(0,c.jsx)(n.Suspense,{children:(0,c.jsx)(w,{offline:t})})})})]})}},35434:(e,t,o)=>{o.d(t,{a:()=>n,x:()=>r});const r=()=>!!window.navigator.standalone||window.matchMedia("(display-mode: standalone)").matches,n=()=>!navigator.onLine},36332:(e,t,o)=>{o.d(t,{A:()=>l});var r=o(9950),n=o(23072),a=o(26044),i=o(68704),s=o(44414);const l=e=>{let{data:t}=e;const o=t.version?t.version.changeLog:"",l=t.disableUpgradeReason?t.disableUpgradeReason:"",d=(0,i.E)(o,e=>{h(e)}),c=(0,i.E)(l,e=>{u(e)}),[g,h]=(0,r.useState)(d),[A,u]=(0,r.useState)(c);return(0,s.jsxs)(s.Fragment,{children:[(0,s.jsx)(a.A,{htmlContent:t.version?g:""}),!t.onlineUpgradable&&(0,s.jsxs)(s.Fragment,{children:[(0,s.jsx)(n.A,{}),(0,s.jsx)(a.A,{htmlContent:A})]})]})}},37596:(e,t,o)=>{o.d(t,{Pb:()=>a,V8:()=>g,dy:()=>d,eP:()=>s,g1:()=>c,hx:()=>h,qm:()=>l,xg:()=>A,z5:()=>u});o(9950);var r=o(27379),n=o(44414);const a=e=>Object.keys(e).reduce(function(t,o){return void 0===e[o]?t.push(`${o}=`):t.push(o+"="+encodeURIComponent(e[o])),t},[]).join("&");function i(e){if(null===e||"object"!==typeof e)return e;if(e instanceof Date)return e.toISOString();if(e instanceof RegExp)return e.toString();if(Array.isArray(e))return e.map(i);const t={};return Object.keys(e).sort().forEach(o=>{t[o]=i(e[o])}),t}function s(e,t){const o=i(e),r=i(t);return JSON.stringify(o)===JSON.stringify(r)}function l(e,t){const o=e.startsWith("?"),r=new URLSearchParams(o?e.substring(1):e);t.split(",").map(e=>{r.delete(e)});const n=r.toString();return n?o?`?${n}`:n:""}const d=e=>e.search.length<=0?e.pathname:e.pathname+e.search,c=e=>{window.onbeforeunloadTips=e,window.onbeforeunload=function(){return e}},g=()=>{window.onbeforeunload=null,window.onbeforeunloadTips=null},h=(e,t)=>{null!==window.onbeforeunload&&(t.warning({title:"\u63d0\u793a",icon:(0,n.jsx)(r.A,{}),content:window.onbeforeunloadTips}),e.preventDefault())},A=["rgb(22, 119, 255)","rgb(114, 46, 209)","rgb(19, 194, 194)","rgb(82, 196, 26)","rgb(235, 47, 150)","rgb(245, 34, 45)","rgb(250, 140, 22)","rgb(250, 219, 20)","rgb(250, 84, 28)","rgb(47, 84, 235)","rgb(250, 173, 20)","rgb(160, 217, 17)","rgb(0, 0, 0)"],u=()=>new URL(document.baseURI).pathname},38692:(e,t,o)=>{o.r(t),o.d(t,{API_DO_UPGRADE_PATH:()=>k,API_VERSION_PATH:()=>B,default:()=>E});var r=o(9950),n=o(20212),a=o(58755),i=o(41988),s=o(65957),l=o(28878),d=o(87094),c=o(52795),g=o(44361),h=o(78816),A=o(13062),u=o(37596),m=o(34647),w=o(63251),p=o(23236),y=o(36332),b=o(26044),f=o(68704),j=o(44414);const{Step:x}=c.A,B="/api/public/version",k="/api/admin/upgrade/doUpgrade",E=e=>{let{data:t,offline:o,offlineData:B}=e;const k=t.preUpgradeKey,E=[{title:(0,h.jY)().changeLog,alias:"changeLog"},{title:"\u4e0b\u8f7d\u66f4\u65b0",alias:"downloadProcess"},{title:"\u6267\u884c\u66f4\u65b0",alias:"doUpgrade"}],v=(0,r.useRef)(null),[Q,H]=(0,r.useState)({current:0,downloadProcess:0,upgradeMessage:""}),{modal:S}=n.A.useApp(),[I,O]=s.Ay.useMessage({maxCount:3}),C=e=>{(0,w.H)(e,K).then(t=>{let{data:o}=t;e!==o.buildId?v.current=setTimeout(()=>{C(e)},500):S.success({title:o.message,content:"",onOk:function(){window.location.href=(0,h.dW)((0,u.z5)()+"admin/index?buildId="+e)}})}).catch(()=>{v.current=setTimeout(()=>{C(e)},500)})},K=(0,m.cz)(),T=async()=>{H(e=>({...e,current:1}));try{const e=await(0,w.b)("/upgrade/download?preUpgradeKey="+k,K);if(e.error)return void I.error(e.message);H(t=>({...t,downloadProcess:e.data.process,current:1})),e.data.process<100&&(v.current=setTimeout(T,500))}catch(e){e instanceof A.pe&&e.response&&e.response.data&&I.error(e.response.data.message)}},D=t.version.buildId,U=async()=>{H(e=>({...e,current:2}));try{const{data:e}=await(0,w.b)("/upgrade/doUpgrade?preUpgradeKey="+k,K);if(e&&e.message){const t=await(0,f.u)(e.message);H(e=>({...e,upgradeMessage:t,current:2}))}if(e&&!e.finish)return void(v.current=setTimeout(U,500));C(D)}catch(e){console.error(e),C(D)}},J=()=>!t.onlineUpgradable;return(0,r.useEffect)(()=>()=>{v&&v.current&&clearTimeout(v.current)},[]),(0,j.jsxs)(d.A,{children:[O,(0,j.jsxs)(i.A,{style:{maxWidth:600},xs:24,children:[(0,j.jsx)(p.A,{title:(0,h.jY)().upgradeWizard}),(0,j.jsx)(c.A,{current:Q.current,style:{paddingTop:16},children:E.map(e=>"downloadProcess"===e.alias&&J()?(0,j.jsx)(j.Fragment,{}):(0,j.jsx)(x,{title:e.title},e.alias))}),(0,j.jsxs)("div",{className:"steps-content",style:{marginTop:"20px"},children:[0===Q.current&&(0,j.jsxs)(j.Fragment,{children:[(0,j.jsx)(g.A,{level:4,children:(0,h.jY)().changeLog}),(0,j.jsx)(y.A,{data:t})]}),1===Q.current&&(0,j.jsxs)(j.Fragment,{children:[(0,j.jsx)(g.A,{level:4,children:"\u4e0b\u8f7d\u66f4\u65b0\u5305"}),(0,j.jsx)(l.A,{strokeLinecap:"round",percent:Q.downloadProcess})]}),2===Q.current&&(0,j.jsxs)(j.Fragment,{children:[(0,j.jsx)(g.A,{level:4,children:"\u6b63\u5728\u6267\u884c\u66f4\u65b0..."}),(0,j.jsx)(b.A,{htmlContent:Q.upgradeMessage})]})]}),(0,j.jsx)("div",{className:"steps-action",style:{paddingTop:"20px"},children:Q.current<E.length-1&&(0,j.jsx)(a.Ay,{type:"primary",loading:B,disabled:!!B||!!o||!t.upgrade||1===Q.current&&Q.downloadProcess<100,onClick:()=>(async()=>{0===Q.current?J()?await U():await T():1===Q.current&&await U()})(),children:(0,h.jY)().nextStep})})]})]},t.preUpgradeKey)}},59922:(e,t,o)=>{o.r(t),o.d(t,{LoginBg:()=>C,StyledLoginPage:()=>H,classes:()=>Q,default:()=>K});var r=o(9950),n=o(15325),a=o(58755),i=o(41988),s=o(57686),l=o(62618),d=o(68832),c=o(65957),g=o(16471),h=o(66577),A=o(78816),u=o(28429),m=o(44361),w=o(31743),p=o(85221),y=o(24937),b=o(37596),f=o(34647),j=o(63251),x=o(11113),B=o(77163),k=o(44414);const E=o(80069),v="login",Q={bg:`${v}-bg`,title:`${v}-title`,card:`${v}-card`,content:`${v}-content`,copyrightTips:`${v}-copyrightTips`},H=(0,y.Ay)(d.A)(e=>{let{mainColor:t}=e;return{height:"100vh",[`& .${Q.bg}`]:{objectFit:"cover",opacity:.6,filter:"blur(1px)",width:"100%",display:"flex",flexDirection:"column",borderRadius:"8px 8px 0 0",alignItems:"center",textAlign:"center",backgroundSize:"cover",height:"180px"},[`& .${Q.title}`]:{color:"#fff",background:t,margin:0,width:"100%",position:"absolute",bottom:0,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",paddingLeft:12,paddingRight:12},[`& .${Q.card}`]:{textAlign:"center",width:"100%",marginRight:"auto",marginLeft:"auto",maxWidth:"660px"},[`& .${Q.content}`]:{minWidth:"100%",display:"flex",alignItems:"center",paddingRight:24,paddingLeft:24},[`& .${Q.copyrightTips}`]:{textAlign:"center",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}}),S={labelCol:{span:8},wrapperCol:{span:8}},{Content:I,Footer:O}=d.A,C=()=>(0,k.jsx)("img",{alt:"bg",src:"data:image/png;base64,",className:Q.bg}),K=e=>{let{offline:t}=e;const[o,d]=(0,r.useState)(!1),y=(0,A.NO)(),[v,K]=(0,r.useState)(y),[T,D]=c.Ay.useMessage({maxCount:3}),U=(0,u.Zp)(),J=(0,f.cz)(),N=async()=>{d(!0);const e={userName:v.userName,password:E(v.password),https:"https:"===window.location.protocol};try{(0,A.M$)()&&(J.defaults.baseURL=v.backendServerUrl);const{data:t}=await J.post("/api/admin/login",e);if(t.error)return void await T.error(t.message);if(0==t.error){(0,A.M$)()&&null!=v.backendServerUrl&&(0,A.PA)(v.backendServerUrl),await(async(e,t)=>{var o;(0,x.TX)().pageBuildId=t.pageBuildId,(0,x.TX)().user=t.data,(0,p.ls)("/user",t.data),(0,A.M$)()&&localStorage.setItem(x.mX,t.data.key);const r=(null!==(o=t.data.cacheableApiUris)&&void 0!==o?o:[]).map(async t=>{const o=t.split("/api/admin")[1],{data:r}=await(0,j.b)(o,e);(0,p.ls)(o,r)});await Promise.all(r).then(()=>{console.log("all api data cached")}).catch(e=>{console.error("cache error\uff1a",e)})})(J,t);const e=new URLSearchParams(window.location.search).get("redirectFrom");if(null!==e&&""!==e){const t=decodeURIComponent(e);if(t.startsWith((0,b.z5)()+"admin/plugins/"))window.location.href=window.location.protocol+"//"+window.location.host+t;else{const e=t.replace((0,b.z5)()+"admin","").split("?");"/"!==e[0]&&""!==e[0]||(e[0]="/index"),U((0,A.dW)(e.join("?")),{replace:!0})}}else U((0,A.dW)("/index"),{replace:!0});return}await T.error("\u672a\u77e5\u9519\u8bef")}finally{d(!1)}};return(0,r.useEffect)(()=>{(0,p.$K)()},[]),(0,k.jsxs)(w.A,{children:[D,(0,k.jsxs)(H,{mainColor:(0,B.BV)().colorPrimary,children:[(0,k.jsx)(I,{className:Q.content,children:(0,k.jsx)(g.A,{cover:(0,k.jsxs)("div",{style:{textAlign:"center",position:"relative"},children:[(0,k.jsx)(C,{}),(0,k.jsx)(m.A,{level:3,className:Q.title,children:(0,A.jY)().userNameAndPassword})]}),className:Q.card,styles:{body:{margin:16}},children:(0,k.jsxs)(s.A,{...S,initialValues:y,onFinish:()=>N(),onValuesChange:(e,t)=>K(t),children:[(0,A.M$)()&&(0,k.jsx)(s.A.Item,{label:(0,A.jY)().backendServerUrl,name:"backendServerUrl",rules:[{required:!0}],children:(0,k.jsx)(l.A,{autoComplete:"url"})}),(0,k.jsx)(s.A.Item,{label:(0,A.jY)().userName,name:"userName",rules:[{required:!0}],children:(0,k.jsx)(l.A,{autoComplete:"username"})}),(0,k.jsx)(s.A.Item,{label:(0,A.jY)().password,name:"password",rules:[{required:!0}],children:(0,k.jsx)(l.A.Password,{autoComplete:"current-password"})}),(0,k.jsx)(h.A,{style:{alignItems:"center",display:"flex"},children:(0,k.jsx)(i.A,{xxl:24,xs:24,children:(0,k.jsxs)(a.Ay,{disabled:t,loading:o,type:"primary",style:{minWidth:108},htmlType:"submit",children:[(0,k.jsx)(n.A,{})," ",(0,A.jY)().login]})})})]})})}),(0,k.jsxs)(O,{className:Q.copyrightTips,children:[(0,A.jY)().copyrightCurrentYear," ",(0,A.jY)().websiteTitle,". All Rights Reserved."]})]})]})}},63251:(e,t,o)=>{o.d(t,{H:()=>a,b:()=>n});var r=o(11113);const n=async(e,t)=>{const{data:o}=await t.get("/api/admin"+e.replace(".html",""));return void 0!==o.pageBuildId&&((0,r.TX)().pageBuildId=o.pageBuildId,(0,r.TX)().systemNotification=o.systemNotification,""!==(0,r.$L)()&&null!==(0,r.$L)()&&void 0!==(0,r.$L)()||(0,r.l6)(o.pageBuildId)),o},a=async(e,t)=>{const{data:o}=await t.get("/api/public/version?buildId="+e);return o}},68704:(e,t,o)=>{o.d(t,{u:()=>y,E:()=>b});var r=o(89346);o(86470);const n=/\$(.+?)\$/g,a=/\$\$([\s\S]+?)\$\$/g;var i=o(21863),s=o(19634),l=o.n(s),d=o(72952),c=o.n(d),g=o(1352),h=o(44414);const A=new Map;const u=()=>{const e=document.createElement("div");return e.style.position="absolute",e.style.left="-9999px",e.style.top="-9999px",e.style.visibility="hidden",e};function m(e,t){return new Promise(o=>{const r=u();document.body.appendChild(r),function(e,t){let o=A.get(e);o||(o=(0,g.H)(e),A.set(e,o)),o.render(t)}(r,(0,h.jsx)(c(),{input:t,options:{theme:"simple"}}));const n=new MutationObserver(()=>{e.innerHTML=r.innerHTML,n.disconnect(),o()});n.observe(r,{childList:!0,subtree:!0}),setTimeout(()=>{n.disconnect(),o()},1e3)})}async function w(e){e.querySelectorAll(".flow").forEach(e=>{const t=decodeURIComponent(e.dataset.code||"");try{const o=u();document.body.appendChild(o);l().parse(t).drawSVG(o),e.innerHTML=o.innerHTML,document.body.removeChild(o)}catch(o){e.innerHTML=`<pre style="color:red">${String(o)}</pre>`}});const t=[];e.querySelectorAll(".seq").forEach(e=>{const o=decodeURIComponent(e.dataset.code||"");try{t.push(m(e,o))}catch(r){e.innerHTML=`<pre style="color:red">${String(r)}</pre>`}}),await Promise.all(t)}const p=e=>{const t=(e=>(e=e.replace(a,(e,t)=>r.Ay.renderToString(t,{displayMode:!0,throwOnError:!1}))).replace(n,(e,t)=>r.Ay.renderToString(t,{displayMode:!1,throwOnError:!1})))((0,i.xI)(e)),o=document.createElement("div");return o.innerHTML=t,o},y=async e=>{const t=p(e);return await w(t),t.innerHTML},b=(e,t)=>{const o=p(e);return w(o).then(()=>{t(o.innerHTML)}),o.innerHTML}},77163:(e,t,o)=>{o.d(t,{xg:()=>J,Ay:()=>G,BV:()=>R});var r=o(79771),n=o(29063),a=o(20212),i=o(9950),s=o(35434),l=o(37596),d=o(47475),c=o(1884),g=o(84560),h=o(42074),A=o(28429),u=o(34647),m=o(78816),w=o(91887),p=o(16471),y=o(57686),b=o(62618),f=o(41988),j=o(58755),x=o(66577),B=o(15325),k=o(59922),E=o(44414);const v={labelCol:{span:8},wrapperCol:{span:14}},Q=e=>{let{onSubmit:t,lang:o}=e;const r="/"!=(0,m.XW)()?(0,m.XW)():window.location.origin,[n,a]=(0,i.useState)(r);return(0,E.jsx)(k.StyledLoginPage,{mainColor:R().colorPrimary,children:(0,E.jsx)(w.UC,{className:k.classes.content,children:(0,E.jsx)(p.A,{className:k.classes.card,cover:(0,E.jsx)(k.LoginBg,{}),styles:{body:{margin:16}},children:(0,E.jsxs)(y.A,{...v,initialValues:{backendServerUrl:n},onFinish:()=>{n.endsWith("/")?t(n):t(n+"/")},onValuesChange:e=>{a(e.backendServerUrl)},children:[(0,E.jsx)(y.A.Item,{label:"zh_CN"===o?"\u7f51\u5173 URL":"Getaway URL",name:"backendServerUrl",rules:[{required:!0}],children:(0,E.jsx)(b.A,{})}),(0,E.jsx)(x.A,{style:{alignItems:"center",display:"flex"},children:(0,E.jsx)(f.A,{xxl:24,xs:24,children:(0,E.jsxs)(j.Ay,{type:"primary",style:{minWidth:108},htmlType:"submit",children:[(0,E.jsx)(B.A,{})," ",(()=>{var e,t;return"zh_CN"===o?null===(t=d.A.Tour)||void 0===t?void 0:t.Next:null===(e=c.A.Tour)||void 0===e?void 0:e.Next})()]})})})]})})})})};var H=o(71420),S=o(3127);const I=()=>{const e=(0,m.jY)().admin_color_primary;return void 0===e||0===e.length?"#1677ff":e},O=()=>{const e=(0,m.jY)().admin_darkMode;return void 0!==e?e:"dark"===(window.matchMedia?(window.matchMedia("(prefers-color-scheme: dark)").matches,"dark"):"light")},C=()=>!0===(0,m.jY)().admin_compactMode,K=e=>{let{lang:t,offline:o}=e;const[r,n]=(0,i.useState)({resLoaded:!1,resLoadErrorMsg:"",requiredBackendServerUrl:!1}),a=(0,u.cz)(),l=e=>{const t="/api/public/adminResource";e.length>0&&(a.defaults.baseURL=e),a.get(t).then(t=>{let{data:o}=t;e.length>0&&(0,m.PA)(e),d(o.data)}).catch(e=>{const o="Request "+a.defaults.baseURL+t.substring(1)+" error -> "+e.message;(0,m.M$)()?n(e=>({...e,resLoaded:!0,requiredBackendServerUrl:!0})):n(e=>({...e,resLoadErrorMsg:o,resLoaded:!1}))})},d=e=>{e.copyrightTips=e.copyright+' <a target="_blank" href="https://blog.zrlog.com/about.html?footer">ZrLog</a>',void 0!==window.inited&&null!==window.inited||((0,m.ji)(e),J({offline:(0,s.a)(),lang:e.lang,dark:O(),colorPrimary:I(),compactMode:C()}),window.inited=!0),n(e=>({...e,resLoadErrorMsg:"",resLoaded:!0,requiredBackendServerUrl:!1}))};return(0,i.useEffect)(()=>{(()=>{const e=(0,m.jY)();null===e||0===Object.keys(e).length?l(""):d(e)})()},[]),r.requiredBackendServerUrl?(0,E.jsx)(Q,{lang:t,onSubmit:e=>{l(e)}}):r.resLoaded?(0,E.jsx)(u.Ay,{offline:o}):0===r.resLoadErrorMsg.length?(0,E.jsx)(H.A,{delay:1e3,fullscreen:!0}):(0,E.jsx)(S.default,{code:500,data:{message:r.resLoadErrorMsg},style:{width:"100vw",height:"100vh"}})},{darkAlgorithm:T,defaultAlgorithm:D,compactAlgorithm:U}=r.A,J=e=>{window.changeAppState(e)};let N={lang:document.documentElement.lang?document.documentElement.lang:"zh_CN",dark:O(),colorPrimary:I(),offline:(0,s.a)(),compactMode:C()};const R=()=>(console.info(N),N),G=()=>{const[e,t]=(0,i.useState)(N),o=()=>{t(e=>({...e,offline:(0,s.a)()}))};(0,i.useEffect)(()=>(window.changeAppState=e=>{t(t=>(N={...t,...e},N))},window.addEventListener("online",o),window.addEventListener("offline",o),()=>{window.removeEventListener("online",o),window.removeEventListener("offline",o)}),[]);const r=(0,l.z5)()+"admin";(0,i.useEffect)(()=>{window.document.body.setAttribute("class",e.dark?"dark":"light")},[e.dark]);const u=[];return e.dark?u.push(T):u.push(D),e.compactMode&&u.push(U),(0,E.jsx)(n.Ay,{locale:e.lang.startsWith("zh")?d.A:c.A,theme:{algorithm:u,token:{colorPrimary:e.colorPrimary}},componentSize:e.compactMode?"small":void 0,table:{style:{whiteSpace:"nowrap"}},divider:{style:{margin:"16px 0"}},card:{styles:{header:{padding:"0 8px",lineHeight:"24px",minHeight:e.compactMode?36:42},body:{padding:8}}},children:(0,E.jsx)(a.A,{children:(0,E.jsx)(g.N7,{transformers:[g.NZ],children:(0,E.jsx)(h.Kd,{basename:r,future:{v7_relativeSplatPath:!0,v7_startTransition:!0},children:(0,E.jsx)(A.BV,{children:(0,E.jsx)(A.qh,{path:"/*",element:(0,E.jsx)(K,{lang:e.lang,offline:e.offline,onInit:e=>{t(t=>({...t,...e}))}})})})})})})})}},78816:(e,t,o)=>{o.d(t,{A_:()=>u,Ay:()=>y,Cu:()=>c,I4:()=>d,M$:()=>g,NO:()=>i,PA:()=>h,Pe:()=>l,XW:()=>A,dW:()=>m,iR:()=>p,jY:()=>a,jb:()=>w,ji:()=>s});var r=o(37596),n=o(11113);const a=()=>{const e=(0,n.TX)().resourceInfo;return void 0===e||null===e?{}:e},i=()=>{const e=a().defaultLoginInfo;if(void 0===e||null===e)return{userName:"",password:"",backendServerUrl:A()};const t=e.backendServerUrl;return void 0===t||null===t||0===t.length?{...e,backendServerUrl:A()}:e},s=e=>{(0,n.TX)().resourceInfo=e},l="_t",d="_t,v",c=()=>!1,g=()=>!!c()||(!!window.location.pathname.endsWith(".html")||a().staticPage),h=e=>{window.localStorage.setItem("_backend_server_url",e)},A=()=>{const e=window.localStorage.getItem("_backend_server_url");return e?e.endsWith("/")?e:e+"/":(0,r.z5)()},u=e=>g()&&e.startsWith("/")?A()+e.substring(1):e,m=e=>{const t=(0,n.TX)().pageBuildId;let o;const r=e.split("?");if(r.length>1){const e=r[1];o=new URLSearchParams(e)}else o=new URLSearchParams("");t&&t.length>0&&o.set("v",t);let a=g()?".html":"";const i=r[0];return i.endsWith(".html")&&(a=""),i+a+"?"+o.toString()},w="/api/admin/article/create",p="/api/admin/article/update",y=class{static getFillBackImg(){return"data:image/png;base64,"}}},85221:(e,t,o)=>{o.d(t,{$K:()=>p,Em:()=>k,H7:()=>u,HB:()=>v,Pn:()=>f,Rn:()=>m,cw:()=>E,f6:()=>j,hS:()=>A,jq:()=>B,ls:()=>b});var r=o(83966),n=o(35434),a=o(37596),i=o(78816),s=o(11113);function l(e,t){const o=(new TextEncoder).encode(e),n=(new TextEncoder).encode(t),a=new Uint8Array(32);a.set(n.subarray(0,32));return function(e){let t="";const o=8192;for(let r=0;r<e.length;r+=o){const n=e.subarray(r,r+o);t+=String.fromCharCode.apply(null,n)}return btoa(t)}(new r.ModeOfOperation.ctr(a).encrypt(o))}function d(e,t){const o=(new TextEncoder).encode(t),n=new Uint8Array(32);n.set(o.subarray(0,32));const a=function(e){const t=atob(e),o=t.length,r=new Uint8Array(o);for(let n=0;n<o;n++)r[n]=t.charCodeAt(n);return r}(e),i=new r.ModeOfOperation.ctr(n).decrypt(a);return(new TextDecoder).decode(i)}const c=()=>(0,s.TX)()&&(0,s.TX)().key?(0,s.TX)().key:"__DEV__DEV__DEV_",g=()=>window.location.host+"_encrypt_page_data";function h(e,t){let o=arguments.length>2&&void 0!==arguments[2]?arguments[2]:" ";if(e.length>=t)return e.substring(0,t);{const r=t-e.length;return e+o.repeat(r)}}const A=e=>m(e.pathname,e.search),u=()=>void 0!==(0,s.TX)().pageBuildId?(0,s.TX)().pageBuildId:"",m=(e,t)=>{let o=e.replace(".html","");return"/"===o&&(o="/index"),o+(0,a.qm)(t,i.I4)},w=()=>{const e=localStorage.getItem(g());try{if(e&&e.length>0)return JSON.parse(d(e,h(c(),24)))}catch(t){console.error(t)}return{}},p=()=>{try{localStorage.removeItem(g()),localStorage.removeItem(s.mX)}catch(e){console.error(e)}},y=e=>{try{localStorage.setItem(g(),l(JSON.stringify(e),h(c(),24)))}catch(t){console.error(t)}},b=(e,t)=>{const o=w();o[e]=t,y(o)},f=e=>w()[e],j=e=>{const t=w();delete t[e],y(t)},x=e=>(0,n.x)()?e+"_page_fullScreen_pwa":e+"_page_fullScreen_normal",B=(e,t)=>{const o=w();o[x(e)]=t,y(o)},k=e=>!0===w()[x(e)],E=e=>{const t=w();t.lastOpenedPage=e,y(t)},v=()=>w().lastOpenedPage}},t={};function o(r){var n=t[r];if(void 0!==n)return n.exports;var a=t[r]={exports:{}};return e[r].call(a.exports,a,a.exports,o),a.exports}o.m=e,(()=>{var e=[];o.O=(t,r,n,a)=>{if(!r){var i=1/0;for(c=0;c<e.length;c++){r=e[c][0],n=e[c][1],a=e[c][2];for(var s=!0,l=0;l<r.length;l++)(!1&a||i>=a)&&Object.keys(o.O).every(e=>o.O[e](r[l]))?r.splice(l--,1):(s=!1,a<i&&(i=a));if(s){e.splice(c--,1);var d=n();void 0!==d&&(t=d)}}return t}a=a||0;for(var c=e.length;c>0&&e[c-1][2]>a;c--)e[c]=e[c-1];e[c]=[r,n,a]}})(),o.n=e=>{var t=e&&e.__esModule?()=>e.default:()=>e;return o.d(t,{a:t}),t},(()=>{var e,t=Object.getPrototypeOf?e=>Object.getPrototypeOf(e):e=>e.__proto__;o.t=function(r,n){if(1&n&&(r=this(r)),8&n)return r;if("object"===typeof r&&r){if(4&n&&r.__esModule)return r;if(16&n&&"function"===typeof r.then)return r}var a=Object.create(null);o.r(a);var i={};e=e||[null,t({}),t([]),t(t)];for(var s=2&n&&r;("object"==typeof s||"function"==typeof s)&&!~e.indexOf(s);s=t(s))Object.getOwnPropertyNames(s).forEach(e=>i[e]=()=>r[e]);return i.default=()=>r,o.d(a,i),a}})(),o.d=(e,t)=>{for(var r in t)o.o(t,r)&&!o.o(e,r)&&Object.defineProperty(e,r,{enumerable:!0,get:t[r]})},o.f={},o.e=e=>Promise.all(Object.keys(o.f).reduce((t,r)=>(o.f[r](e,t),t),[])),o.u=e=>"static/js/"+e+"."+{78:"836172e6",194:"43eaad31",233:"79228981",298:"93db21ee",761:"616c0fa5",764:"49b5fed0",959:"9cff7699",1082:"a3f7e440",1149:"e3188d13",1284:"1b4d60ad",1431:"8d50f157",1507:"246888df",1525:"d773b450",1567:"77410ced",1739:"8d5c058d",1799:"86f77618",1817:"d8259cfc",1977:"3d2aded5",2002:"96a6bd86",2029:"c911b168",2077:"a3afc2f5",2083:"bb501fa3",2092:"f38ba127",2134:"eed82091",2235:"b83e2340",2237:"e3587429",2265:"7a0f028f",2272:"d0a75a7b",2280:"71abaf0b",2378:"ab609c81",2405:"cd34de99",2468:"86408b6d",2504:"ce40e8f9",2515:"b295d759",2638:"e1e03bec",2816:"316c0826",2879:"e481d576",2943:"dd039350",2977:"0b2b8005",3032:"01bfad37",3219:"906653ae",3286:"12c7e565",3287:"575404e6",3319:"3934771d",3338:"ac2f8d57",3403:"7945f302",3605:"207dabb2",3642:"01ab776a",3646:"6466034e",3750:"d7de4f28",3925:"e3d58c9e",4092:"98dbb46c",4222:"d5180df4",4312:"ef2dc66a",4449:"067a460e",4482:"dcda5667",4725:"9c45b440",4868:"8e9a3c42",5053:"75631055",5079:"906e1be8",5110:"d2b839c9",5112:"afb2f2ff",5143:"8e70aca6",5149:"976b0ecc",5179:"630efbc7",5250:"5c5760a1",5306:"7db8f6e0",5389:"34ff50ff",5500:"695ee115",5594:"98c30dad",5648:"8a84d7a8",5682:"6753f52e",5784:"9e918b73",5807:"5f1b55e9",5935:"cad0202f",6011:"78e9542a",6052:"30e23eca",6082:"5e6b9ab3",6296:"2e46d306",6361:"5520f997",6550:"08203edc",6555:"d855df9b",6684:"077f83fe",6722:"e6663c71",6919:"ed789eb3",6930:"c730c2ce",6967:"686ef684",7334:"842f940c",7530:"67283a7a",7601:"a5c46139",7702:"f5ba7dd9",7728:"1c491466",7803:"ea4076e2",7881:"847f238b",7926:"e782477b",7969:"b9694e95",8116:"6a6a2b5f",8197:"7100a412",8411:"c3cb2ac5",8590:"155e8785",8836:"5c4475f4",8868:"2ecb17bb",8883:"1669e588",8971:"fe4aa312",9011:"dc62d3d6",9072:"e64cb0ce",9107:"4590d572",9273:"9d9a5342",9431:"df9274cf",9478:"77704b98",9493:"db589acf",9538:"8c90ea7b",9675:"d7eb950b",9984:"fa4b6a9b",9997:"51418726"}[e]+".chunk.js",o.miniCssF=e=>{},o.g=function(){if("object"===typeof globalThis)return globalThis;try{return this||new Function("return this")()}catch(e){if("object"===typeof window)return window}}(),o.o=(e,t)=>Object.prototype.hasOwnProperty.call(e,t),(()=>{var e={},t="admin-web-frontend:";o.l=(r,n,a,i)=>{if(e[r])e[r].push(n);else{var s,l;if(void 0!==a)for(var d=document.getElementsByTagName("script"),c=0;c<d.length;c++){var g=d[c];if(g.getAttribute("src")==r||g.getAttribute("data-webpack")==t+a){s=g;break}}s||(l=!0,(s=document.createElement("script")).charset="utf-8",s.timeout=120,o.nc&&s.setAttribute("nonce",o.nc),s.setAttribute("data-webpack",t+a),s.src=r),e[r]=[n];var h=(t,o)=>{s.onerror=s.onload=null,clearTimeout(A);var n=e[r];if(delete e[r],s.parentNode&&s.parentNode.removeChild(s),n&&n.forEach(e=>e(o)),t)return t(o)},A=setTimeout(h.bind(null,void 0,{type:"timeout",target:s}),12e4);s.onerror=h.bind(null,s.onerror),s.onload=h.bind(null,s.onload),l&&document.head.appendChild(s)}}})(),o.r=e=>{"undefined"!==typeof Symbol&&Symbol.toStringTag&&Object.defineProperty(e,Symbol.toStringTag,{value:"Module"}),Object.defineProperty(e,"__esModule",{value:!0})},o.p="/admin/",(()=>{var e={8792:0};o.f.j=(t,r)=>{var n=o.o(e,t)?e[t]:void 0;if(0!==n)if(n)r.push(n[2]);else{var a=new Promise((o,r)=>n=e[t]=[o,r]);r.push(n[2]=a);var i=o.p+o.u(t),s=new Error;o.l(i,r=>{if(o.o(e,t)&&(0!==(n=e[t])&&(e[t]=void 0),n)){var a=r&&("load"===r.type?"missing":r.type),i=r&&r.target&&r.target.src;s.message="Loading chunk "+t+" failed.\n("+a+": "+i+")",s.name="ChunkLoadError",s.type=a,s.request=i,n[1](s)}},"chunk-"+t,t)}},o.O.j=t=>0===e[t];var t=(t,r)=>{var n,a,i=r[0],s=r[1],l=r[2],d=0;if(i.some(t=>0!==e[t])){for(n in s)o.o(s,n)&&(o.m[n]=s[n]);if(l)var c=l(o)}for(t&&t(r);d<i.length;d++)a=i[d],o.o(e,a)&&e[a]&&e[a][0](),e[a]=0;return o.O(c)},r=self.webpackChunkadmin_web_frontend=self.webpackChunkadmin_web_frontend||[];r.forEach(t.bind(null,0)),r.push=t.bind(null,r.push.bind(r))})(),o.nc=void 0;var r=o.O(void 0,[4121,886],()=>o(15246));r=o.O(r)})();